...
// --- Register Endpoint ---
// No Multer middleware needed since we receive JSON Base64
router.post("/register", async (req, res, next) => {
    try {
        const { email, password, name, role, faculty, npm, username, profileImage } = req.body
        ...
        const user = new User({
            ...
            profileImage, // Base64 string directly from JSON
        })
...
router.get("/me", async (req, res) => {
    try {
        ...
        const user = await User.findById(payload.userId).select("email name role faculty npm username profileImage")
